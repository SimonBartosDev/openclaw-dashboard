...
function ensureKanban() {
  ...
  if (!fs.existsSync(KANBAN_PATH)) {
    const seed = {
      columns: {
        backlog: [
          { id: 't-1', title: 'Dashboard v1: Live OpenClaw metrics', owner: 'Builder' },
          { id: 't-2', title: 'Wire Review + Audit gates', owner: 'Reviewer/Auditor' }
        ],
        in_progress: [
          { id: 't-3', title: 'Kanban board + workflow controls', owner: 'Builder' }
        ],
        review: [],
        done: []
      }
    };
    fs.writeFileSync(KANBAN_PATH, JSON.stringify(seed, null, 2));
  }
}

export function getKanban() {
  ensureKanban();
  return readJsonSafe(KANBAN_PATH, { columns: { backlog: [], in_progress: [], review: [], done: [] } });
}
